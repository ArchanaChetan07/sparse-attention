...
if __name__ == "__main__":
    # Write the file ourselves rather than relying on a shell redirect:
    # PowerShell's `>` defaults to UTF-16, which is how TABLES.md previously
    # landed in the repo unreadable on GitHub. --out makes encoding ours.
    import argparse
    import contextlib
    import io
    ...
    ap.add_argument("--out", default=None,
                    help="write UTF-8 markdown here (default: stdout)")
    a = ap.parse_args()
    if a.out:
        buf = io.StringIO()
        ...
        print(f"wrote {a.out} (utf-8, {len(buf.getvalue())} chars)")
    else:
        emit()
